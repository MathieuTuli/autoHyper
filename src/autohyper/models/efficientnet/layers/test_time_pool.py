    ...
    def __init__(self, base, original_pool=7):
        ...
        if isinstance(base_fc, nn.Conv2d):
            ...
            self.fc.weight.data.copy_(
                base_fc.weight.data.reshape(self.fc.weight.size()))
            self.fc.bias.data.copy_(
                base_fc.bias.data.reshape(self.fc.bias.size()))
...
def apply_test_time_pool(model, config, args):
    ...
    if not args.no_test_pool and \
        ...
        model = TestTimePoolHead(
            model, original_pool=model.default_cfg['pool_size'])
        test_time_pool = True
